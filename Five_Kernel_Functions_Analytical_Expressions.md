...
- $K_{xy} = k(X_{\text{tar}}, X_{\text{tar\_next}})$: Cross-kernel matrix $(n \times n)$
...
Check if kernel matrix is reasonable:
```python
# 1. Diagonal elements
print(f"K_xx diagonal: {np.diag(K_xx)[:5]}")
# RBF, Spherical, Matérn, RQ: should be [1, 1, 1, 1, 1]
# Polynomial: depends on parameters

# 2. Matrix should be symmetric
print(f"Symmetry error: {np.max(np.abs(K_xx - K_xx.T))}")  # Should be close to 0

# 3. Matrix should be positive definite (all eigenvalues > 0)
eigvals = np.linalg.eigvalsh(K_xx)
print(f"Min eigenvalue: {np.min(eigvals)}")  # Should be > 0

# 4. Check condition number (should not be too large)
cond = np.linalg.cond(K_xx)
print(f"Condition number: {cond}")  # Should be < 1e10

# 5. Value range check
print(f"K_xx range: [{np.min(K_xx):.3f}, {np.max(K_xx):.3f}]")
```
...
```python
# Check if values are too large/small
print(f"K_xx mean: {np.mean(K_xx):.3f}")
print(f"K_xx std: {np.std(K_xx):.3f}")

# Typical good ranges (for unit sphere data):
# d=2: mean ~ 1-3, std ~ 0.5-1.5
# d=3: mean ~ 1-5, std ~ 1-3
# If values are too large (>100), reduce gamma or degree
```
